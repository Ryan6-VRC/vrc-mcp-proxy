...
| `execute_code_compile_notes` | tools/call resp | On an `execute_code` compile failure — from `execute` **or `replay`**, which is where a failed entry genuinely recompiles — appends one note per trap for two compile errors lifted out of `docs/unity.md`: a name two pre-imported namespaces both define (`Object`, `Random`), and a type used where a value belongs (usually an attempted static-class alias). Matches both compiler dialects — Roslyn and CodeDom word and quote these incompatibly. Diagnostic only: it never rewrites the payload. |
