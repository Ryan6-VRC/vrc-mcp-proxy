...
| `manage_asset_search_pattern_note` | tools/call resp | On a `manage_asset` `search` whose `search_pattern` carries a file extension, appends the note that `AssetDatabase.FindAssets` matches an asset's name with the **extension excluded** — so a `.mat` in the pattern never selects by type, and where names do embed that literal text it returns confidently wrong hits (measured: `*.fbx` over `Assets` returned 13 assets, every one a `.json` log). Names the cure, `filter_type`. Fires on hits and on zero alike, because the non-zero arm is the dangerous one. Silent on a dotted `t:`/`l:` filter term — upstream's own field description recommends putting those here — and on an ordinary dotted name fragment. Diagnostic only. |
| `manage_asset_search_scope_note` | tools/call resp | On a `manage_asset` `search` whose folder scope upstream dropped, appends whichever of three things establishes it: the scope is one the `Assets/` force-prefix destroys (`Packages/…`, an absolute path, a `..` traversal — decidable from the request alone), or hits came back outside the requested folder, or an empty result sits over a folder that is not on disk in the pinned venue. Upstream sets `folderScope = null` for any path that isn't a valid folder and searches the **whole project**, warning to the Unity console only — the surface this proxy denies. Diagnostic only. |
