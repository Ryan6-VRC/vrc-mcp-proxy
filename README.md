...
| `manage_asset_truth_correction` | tools/call resp | On a move/rename/delete reported as failed, verifies on disk and rewrites a false failure to success. Inferred from disk state, never observed, on both arms — delete only when the asset and its `.meta` are both gone; move when the destination is present and the source absent, which a move whose source never existed also satisfies. The note says so. |
...
| `execute_code_compile_notes` | tools/call resp | On an `execute_code` compile failure, appends one note per trap for two compile errors lifted out of `docs/unity.md`: a name both pre-imported namespaces define (`Object`, `Random`), and a type used where a value belongs (usually an attempted static-class alias). Matches both compiler dialects — Roslyn and CodeDom word and quote these incompatibly. Diagnostic only: it never rewrites the payload. |
| `execute_code_prelude_offset_note` | tools/call resp | On an `execute_code` compile failure, discloses that the reported line numbers count the proxy's own injected prelude (11 lines with both guards live), gives the subtraction, and names the two bands that are not the caller's code at all — the preamble at or below the offset, and the wrapper trailer past their last line, where a snippet with unbalanced braces reports. States the offset rather than correcting it: rewriting third-party compiler prose would emit a wrong line number as fact. Silent when nothing is injected. |
