...
| `execute_code_compile_notes` | tools/call resp | On an `execute_code` compile failure — from `execute` **or `replay`**, which is where a failed entry genuinely recompiles — appends one note per trap for four compile errors: a name two pre-imported namespaces both define (`Object`, `Random`), a type used where a value belongs (usually an attempted static-class alias), a name that resolved to nothing — a typo, or a type outside the six pre-imported namespaces used unqualified (the agent/avatar tool doors and `UnityEditor.SceneManagement` are the common ones) — and a member that does not exist on a type that does. The first two were lifted out of `docs/unity.md`; the last two that doc never described. Matches both compiler dialects — Roslyn and CodeDom word and quote the first two incompatibly, and differ only in quoting on the third and fourth. The fourth fires only when the receiver is one of this workspace's own `[AgentTool]` classes, read from the pinned venue's own installed packages (`kits.py`), and it **routes rather than prescribes**: it names the owning kit and sends the reader to that door's row in `docs/unity-tools.md` instead of guessing which door was meant. Diagnostic only: it never rewrites the payload. |
