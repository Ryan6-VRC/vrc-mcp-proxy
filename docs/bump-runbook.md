...
5. **Re-run the repro driver.** Against a scratch Editor (it creates/deletes `Assets/A10Repro`):
   ...
   ```
   Re-confirm each verdict: does G22 double-execute still reproduce (idempotency guard still earns its keep)? Does F22 still lie — **both** halves, `F22-move-lies` and `F22b-bare-destination-relocates`, since verdict reporting and argument resolution are separate mechanisms and the `move`/`rename` refusal is earned by either one alone? Then the two `search` rows, and note that these retire **independently** of each other, unlike F22's pair: `F23b-extension-excluded-from-name-match` earns `manage_asset_search_pattern_note` (it fails only when a `Name.ext` pattern finally finds `Name.ext` — its other two probes exist to catch upstream swapping in a *different* wrong matcher, a glob or a dot-splitter, which would keep the trap alive in a new shape while the headline probe read clean), and `F23c-scope-path-dropped` earns `manage_asset_search_scope_note` (it fails only when **both** a nonexistent scope and a `Packages/` scope stop returning project-wide hits). If a failure family is fixed upstream, retire its behavior AND its ledger line in `docs/design.md`.
