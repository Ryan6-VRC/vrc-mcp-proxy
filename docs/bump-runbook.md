...
4. **Re-run the repro driver.** Against a scratch Editor (it creates/deletes `Assets/A10Repro`):
   ...
   ```
   Re-confirm each verdict: does G22 double-execute still reproduce (idempotency guard still earns its keep)? Does F22 still lie (truth-correction still needed)? If a failure family is fixed upstream, retire its behavior AND its ledger line in `docs/design.md`.
